...
function WarehouseHub({ warehouse = [], assets = [], projects = [], settings = {}, suppliers = [], onAdd, onExportExcel, onExportPdf, onImport, onDeleteRows, isAdmin }) {
  ...
  const safeWarehouse = (Array.isArray(warehouse) ? warehouse : []).filter(w => w && typeof w === "object").map((w, i) => ({
    ...
    unit: safeText(w.unit, "Cái"), description: safeText(w.description), note: safeText(w.note),
  ...
  const reportTx=safeWarehouse.filter(w=>!w.date || safeText(w.date).slice(0,10)<=asOf).slice().sort((a,b)=>`${safeText(a.date)}|${safeText(a.voucherNo)}|${safeText(a.id)}`.localeCompare(`${safeText(b.date)}|${safeText(b.voucherNo)}|${safeText(b.id)}`));
  const balances={};
  reportTx.forEach(w=>{
    const a=assetMap[safeText(w.assetId)]||{};
  ...
  return <div className="aa-fade">
    ...
    <div className="rounded-lg overflow-hidden" style={{background:TOKENS.surface,border:`1px solid ${TOKENS.border}`}}><div className="flex border-b overflow-x-auto" style={{borderColor:TOKENS.border}}>{tabBtn("in","Phiếu nhập kho",<PackagePlus size={15}/>)}{tabBtn("out","Phiếu xuất kho",<PackageMinus size={15}/>)}{tabBtn("report","Tổng hợp nhập xuất tồn",<ClipboardList size={15}/>)}{tabBtn("txdetail","Chi tiết nhập xuất tồn",<FileSpreadsheet size={15}/>)}{tabBtn("lotdetail","Tồn chi tiết theo phiếu nhập",<Archive size={15}/>)}</div><div className="p-4">
      {(tab==="in"||tab==="out")&&<><div className="flex items-center justify-between gap-3 mb-3"><div className="flex gap-2"><Btn kind="primary" icon={Plus} onClick={()=>onAdd?.(visibleType)}>{tab==="in"?"Lập phiếu nhập":"Lập phiếu xuất"}</Btn>{visibleRows.length>0&&<label className="inline-flex items-center gap-1.5 text-[12px]"><input type="checkbox" checked={allVisible} onChange={()=>setSelectedRows(allVisible?selectedRows.filter(id=>!visibleRows.some(w=>w.id===id)):[...new Set([...selectedRows,...visibleRows.map(w=>w.id)])])}/> Chọn tất cả trang lọc</label>}</div><div className="flex gap-2"><input className={inputCls} style={{...inputStyle,maxWidth:360}} placeholder="Tìm phiếu, tài sản, NCC, diễn giải..." value={query} onChange={e=>setQuery(e.target.value)}/>{isAdmin&&selectedRows.length>0&&<Btn kind="danger" icon={Trash2} onClick={()=>{onDeleteRows?.(selectedRows);setSelectedRows([])}}>Xóa {selectedRows.length} dòng</Btn>}</div></div><WarehouseFilter {...filterProps}/><div className="rounded-lg overflow-auto" style={{border:`1px solid ${TOKENS.border}`}}><table className="w-full min-w-[2050px]"><thead><tr>{txHeaders.map((h,i)=><Th key={`${i}-${h}`}>{h}</Th>)}</tr></thead><tbody>{visibleRows.map((w,i)=>{const a=assetMap[w.assetId]||{};return <tr key={w.id||`row-${i}`} className="aa-row"><Td><input type="checkbox" checked={selectedRows.includes(w.id)} onChange={e=>setSelectedRows(e.target.checked?[...selectedRows,w.id]:selectedRows.filter(id=>id!==w.id))}/></Td><Td><Tag>{safeText(w.voucherNo,"—")}</Tag></Td><Td mono>{fmtDate(w.date)}</Td><Td>{safeText(w.operationLabel||OPERATION_LABELS[w.operationType],"—")}</Td><Td>{safeText(w.itemName||a.name,"—")}</Td><Td><Tag>{safeText(w.itemCode||a.code,"—")}</Tag></Td><Td>{getLocation(w)}</Td><Td>{safeText(w.counterpartyLocation,"—")}</Td><Td>{safeText(w.supplier||w.repairVendor,"—")}</Td><Td>{safeText(w.category||a.category,"—")}</Td><Td>{safeText(w.assetGroup||a.assetGroup,"—")}</Td><Td>{safeText(w.ownership||a.ownership,"—")}</Td><Td>{safeText(w.receiver,"—")}</Td><Td right mono>{safeNumber(w.quantity)}</Td><Td>{safeText(w.unit||a.unit,"Cái")}</Td><Td right mono>{fmtVND(safeNumber(w.unitCost))}</Td><Td right mono>{fmtVND(Number.isFinite(Number(w.total))?Number(w.total):safeNumber(w.quantity)*safeNumber(w.unitCost))}</Td><Td>{safeText(w.description||w.note,"—")}</Td></tr>})}</tbody></table>{!visibleRows.length&&<EmptyState text={tab==="in"?"Chưa có phiếu nhập phù hợp":"Chưa có phiếu xuất phù hợp"}/>}</div></>}
